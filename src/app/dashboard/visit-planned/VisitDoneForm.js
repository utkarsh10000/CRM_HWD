...
export default function VisitDoneForm({ visit, onSaved }) {
  ...
  const [remark, setRemark] = useState(visit.outcome?.remark || "");
  const [imageFile, setImageFile] = useState(null);
  const [imagePreview, setImagePreview] = useState(visit.outcome?.imageUrl || "");
  const [existingImageUrl, setExistingImageUrl] = useState(visit.outcome?.imageUrl || "");
  const [existingImagePublicId, setExistingImagePublicId] = useState(visit.outcome?.imagePublicId || "");
  ...
  async function handleSubmit(e) {
    ...
    try {
      ...
      console.error("Visit done submit error:", err);
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-4" noValidate>
      ...
      {/* Client Response */}
      <div>
        <label htmlFor="response" className="mb-1.5 block text-sm font-medium text-slate-700">
            ...
            <>
              ...
              >
                <path
                  ...
                  d="M3 16.5V19a1.5 1.5 0 001.5 1.5h15A1.5 1.5 0 0021 19v-2.5M16.5 8.25L12 3.75m0 0L7.5 8.25M12 3.75V15"
                />
              </svg>
      ...
      <button
        type="submit"
        disabled={submitting}
        className="w-full rounded-md bg-green-600 px-3 py-2 text-sm font-medium text-white transition-colors hover:bg-green-700 disabled:cursor-not-allowed disabled:opacity-60"
      >
        {uploading ? "Uploading image…" : submitting ? "Saving…" : "Mark as Done"}
      </button>
    </form>
  );
}